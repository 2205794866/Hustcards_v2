    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    record_model->setHorizontalHeaderLabels(QStringList() << "序号"
                                                          << "时间"
                                                          << "卡号"
                                                          << "金额");
    //设置
    ...
    //判断是否为KH
    {
        ...
        //若未到结尾
        {
            if (afile.getline(buff, 255).good())
            {
                ...
                //开户
                this->CM->open_account(logger.get_time(), stu_ID, name);
            }
            else
            {
                break;
...
void MainWindow::execute(Operation *one)
{
    ...
    else
    {
        ...
        {
        case 1:
            this->CM->cancel_account(one->tm, one->stu_ID);
            break;
        case 2:
            this->CM->reissue_card(one->tm, one->stu_ID);
            break;
        case 3:
...
{
    //获取时间、卡号、密码
    std::string time = logger.get_time();
