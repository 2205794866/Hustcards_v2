...
stu_info::stu_info(CardManager *CM, int func_num, std::string stu_ID, QWidget *parent) : QDialog(parent),
                                                                                         ui(new Ui::stu_info)
{
    //初始化
    ui->setupUi(this);
...
void stu_info::on_buttonBox_accepted()
{
    ...
    if (this->func_num == 1)
    {
        if (this->CM->cancel_account(logger.get_time(), stu_ID) == true)
        {
    ...
    // 恢复账户
    {
        if (this->CM->recover_account(logger.get_time(), stu_ID) == true)
        {
            succeed *ui_succeed = new succeed(this);
            ui_succeed->show();
        }
    ...
    // 补卡
    {
        if (this->CM->reissue_card(logger.get_time(), stu_ID) == true)
        {
            succeed *ui_succeed = new succeed(this);
            ui_succeed->show();
        }
    ...
    // 挂失
    {
        if (this->CM->report_lost(logger.get_time(), stu_ID) == true)
        {
            succeed *ui_succeed = new succeed(this);
            ui_succeed->show();
        }
    ...
    // 解挂
    {
        if (this->CM->remove_lost(logger.get_time(), stu_ID) == true)
        {
            succeed *ui_succeed = new succeed(this);
            ui_succeed->show();
        }
